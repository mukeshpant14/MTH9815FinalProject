...
public:
	...
	void ExecuteOrder(ExecutionOrder<Bond>& order, Market market)
	{
		...
		this->exeuctionOrderMap[productId] = order;

		this->callListeners(order, Action::ADD);
...
public:
	...
	{
		ExecutionOrder<Bond> executionOrder = data.getExecutionOrder();
		this->bondExecutionService->ExecuteOrder(executionOrder, data.getMarket()); 
	};
