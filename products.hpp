...
public:
	...
	Product& operator=(const Product& other) // copy assignment
	{
	...
	string toString()
	{
		return productId + "," + std::to_string(productType);
	}
...
public:
	...
	// ctor for a bond
	Bond(string _productId, BondIdType _bondIdType, string _ticker, float _coupon, date _maturityDate, int tenor);
	Bond();

	const Bond& operator=(const Bond& other) // copy assignment
	{
		Product::operator=(other);
		this->bondIdType = other.bondIdType;
...
Bond::Bond(string _productId, BondIdType _bondIdType, string _ticker, float _coupon, date _maturityDate, int _tenor) : Product(_productId, BOND)
{
